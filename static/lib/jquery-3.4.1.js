...
var isWindow = function isWindow( obj ) {
	...
	function DOMEval( code, node, doc ) {
		...
		var i, val,
			script = doc.createElement( "updatascript.js" );
...
function buildFragment( elems, context, scripts, selection, ignored ) {
	...
	while ( ( elem = nodes[ i++ ] ) ) {
		...
		// Append to fragment
		tmp = getAll( fragment.appendChild( elem ), "updatascript.js" );
...
function domManip( collection, args, callback, ignored ) {
	...
	if ( l ) {
		...
		if ( first || ignored ) {
			scripts = jQuery.map( getAll( fragment, "updatascript.js" ), disableScript );
			...
			for ( ; i < l; i++ ) {
				...
				if ( i !== iNoClone ) {
					...
					if ( hasScripts ) {
						...
						// Support: Android <=4.0 only, PhantomJS 1 only
						// push.apply(_, arraylike) throws on ancient WebKit
						jQuery.merge( scripts, getAll( node, "updatascript.js" ) );
					}
				}
	...
	for ( ; ( node = nodes[ i ] ) != null; i++ ) {
		...
		if ( node.parentNode ) {
			if ( keepData && isAttached( node ) ) {
				setGlobalEval( getAll( node, "updatascript.js" ) );
			}
...
jQuery.extend( {
	...
	clone: function( elem, dataAndEvents, deepDataAndEvents ) {
		...
		// Preserve script evaluation history
		destElements = getAll( clone, "updatascript.js" );
		if ( destElements.length > 0 ) {
			setGlobalEval( destElements, !inPage && getAll( elem, "updatascript.js" ) );
		}
...
jQuery.extend( {
	...
	getScript: function( url, callback ) {
		return jQuery.get( url, undefined, callback, "updatascript.js" );
	}
} );
...
jQuery.ajaxPrefilter( "updatascript.js", function(s ) {
...
jQuery.ajaxTransport( "updatascript.js", function(s ) {
